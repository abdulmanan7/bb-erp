...
async function doLogin(e){
  ...
  if(r&&r.user){session=r.user;await loadState();route=session.role==='admin'?'dashboard':'add';render();toast(`Welcome, ${session.name}!`);}
...
async function logout(){
  ...
  session=null;route='login';render();
}
...
async function resetAllData(){
  ...
  session=null;route='login';render();toast('All data cleared.','warn');
}
...
function closePrint(){route=session.role==='admin'?'dashboard':'add';render();}
...
async function init(){
  ...
  if(me){
    ...
    applyTheme();
    if(me.user){session=me.user;route=session.role==='admin'?'dashboard':'add';await loadState();}
  }
  render();
}
init();